...
import { db } from "../firebaselmp/main";
...
export const AvailableScooters = (params) => {
  ...
  useEffect(() => { 
    let isMounted = true
    const q = query(collection(db, "scooters"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        // if (change.type === "added") {
        //   console.log("New Scooter: ", change.doc.data());
        // }
        // if (change.type === "modified") {
        //     console.log("Modified Scooter: ", change.doc.data());
        // }
        // if (change.type === "removed") {
        //     console.log("Removed Scooter: ", change.doc.data());
        // }
        var scooterdata = change.doc.data()
        if(params.type === "scooterdetail"){
          if(change.doc.id === params.scooterID){
            setScooterStatus(scooterdata)
          }
        }
        if (isMounted) {
          setScootermarker(p => {
            let prevScooters = [...p]
            const prevScooterIndex = p.findIndex(s => s.i === scooterdata.i)
            if (prevScooterIndex === -1) {
              prevScooters.push(scooterdata)
            } else {
